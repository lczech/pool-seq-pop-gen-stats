...
Furthermore, we (re-)derive two consistent (unbiased) estimators for \fst{} for pool sequencing data, using two distinct definition of \fst{}, that correct for both biases described above.
We also evaluate the different estimators for \fst{} here using simulated data, showing their particular biases.
...
$\samplesize$ :
Pool size, provided by the user. This is the number of individuals that were pooled together for sequencing of a population.
...
To this end, \toolname{PoPoolation} uses a modified version of the $Y^*$ test of \citeay{Achaz2008},
...
The effect of those bugs is shown in \figref{fig:bug-exam}. In short, we observed an overestimation of Tajima's D in the range 2.8 to 3.5 times when using the default settings of \toolname{PoPoolation}. We hence recommend to re-run any analyses of Tajima's D that have been conducted with past versions of \toolname{PoPoolation}, in particular if any of their results rely on the exact numerical values.


\todo{more bugs: the last position of a window is accidentally used as the first of the following instead...}
...
We show the results of the simulations in Figures~\ref{fig:UnbiasedHudson}-\ref{fig:Karlsson}. Each Figure shows the comparison of the estimated values versus the true values, for \texttt{msprime} on the left, and for the "simple" simluation on the right. Each plot is repeated three times, colorized by Pool Size, Read Depth, and Sequencing Error, respectively, in order to show the effects of each of these parameters on the estimation.
...
In the following, we present derivations of the above estimators of \fst{} that also correct for sequencing errors. This is currently not implemented in our software, and in no other software that we are aware of. We however present it here for completeness, in the hope that this is useful for others.
