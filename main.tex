...
\newcommand{\fst}{\textit{F}\textsubscript{ST}}
\newcommand{\mathfst}{\text{\textit{F}}_\text{ST}}
\newcommand{\neifst}{\text{\textit{F}}_\text{ST}^\text{Nei}}
\newcommand{\hudsonfst}{\text{\textit{F}}_\text{ST}^\text{Hudson}}
\newcommand{\neiestimator}{\widehat{\text{\textit{F}}}_\text{ST}^\text{Nei}}
\newcommand{\hudsonestimator}{\widehat{\text{\textit{F}}}_\text{ST}^\text{Hudson}}
\newcommand{\eg}{e.\,g.,~}
\newcommand{\ie}{i.\,e.,~}
...
There are several non-equivalent \emph{definitions} of $\text{\textit{F}}_\text{ST}$.
...
There are two natural candidates leading to two related, but distinct definitions of $\text{\textit{F}}_\text{ST}$.
The first definition, which we will call $\text{\textit{F}}_\text{ST}^\text{Nei}$ following \citeay{Nei1973}, considers the proportion of the total variation in the two populations.
This statistic is also called $\text{G}_\text{ST}$, see for example Equation (5.5) of \citeay{Hahn2018}.
The second definition, which we will call $\text{\textit{F}}_\text{ST}^\text{Hudson}$ following \citeay{Hudson1992}, considers the proportion of the variation between populations, see also \citeay{Cockerham1969} and \citeay{Weir2002}.
...
Our two definitions of $\text{\textit{F}}_\text{ST}$ are then

\begin{align}
    \text{\textit{F}}_\text{ST}^\text{Nei}    &:= 1 - \frac{\pi_\text{within}}{\pi_\text{total}} \label{eq:popnei}\\
    \text{\textit{F}}_\text{ST}^\text{Hudson} &:= 1 - \frac{\pi_\text{within}}{\pi_\text{between}} \label{eq:pophudson}
\end{align}
...
Since both definitions of $\text{\textit{F}}_\text{ST}$ rely on these $\pi$s, we will need to derive unbiased estimates for them.
...
\subsection{Final asymptotically unbiased estimators of \texorpdfstring{$\text{\textit{F}}_\text{ST}$}{FST} per SNP and per window}
\label{supp:sec:FST:sub:EstimatorFST}

These estimators then immediately suggest the following ratio estimators for the different definitions of $\text{\textit{F}}_\text{ST}$:

\begin{align}
    \widehat{\text{\textit{F}}}_\text{ST}^\text{Nei} &:= 1 - \frac{\widehat{\pi}_\text{within}}{\widehat{\pi}_\text{total}}\\
    \widehat{\text{\textit{F}}}_\text{ST}^\text{Hudson} &:= 1 - \frac{\widehat{\pi}_\text{within}}{\widehat{\pi}_\text{between}}
\end{align}
...
\begin{align}
    \widehat{\text{\textit{F}}}_\text{ST}^\text{Nei}    &= 1 - \frac{\sum_{\ell \in W} \widehat{\pi}^\ell_\text{within}} {\sum_{\ell \in W} \widehat{\pi}^\ell_\text{total}} \\
    \widehat{\text{\textit{F}}}_\text{ST}^\text{Hudson} &= 1 - \frac{\sum_{\ell \in W} \widehat{\pi}^\ell_\text{within}} {\sum_{\ell \in W} \widehat{\pi}^\ell_\text{between}}
\end{align}
...
\subsection{Estimator of \texorpdfstring{$\text{\textit{F}}_\text{ST}$}{FST} as implemented in PoPoolation2}
...
We start with the definition of $\text{\textit{F}}_\text{ST}^\text{Karlsson}$ from Karlsson \textit{et al.} for the SNPs in a window $W$:

\begin{align}
    \label{eq:FstK}
    \text{\textit{F}}_\text{ST}^\text{Karlsson} &= \frac{\sum_W N_k}{\sum_W D_k}
\end{align}
...
It is not obvious, but follows from simple algebra that this definition is equivalent to $ \text{\textit{F}}_\text{ST}^\text{Hudson} $ defined above.
...
And finally, these are used to define estimator $\widehat{\text{\textit{F}}}_\text{ST}^\text{Karlsson}$ for a window $W$:

\begin{align}
    \label{eq:FstEstK}
    \widehat{\text{\textit{F}}}_\text{ST}^\text{Karlsson} &= \frac{\sum_W \widehat{N}_k}{\sum_W \widehat{D}_k}
\end{align}
...
This estimator is very similar (after some algebra) to our $\widehat{\text{\textit{F}}}_\text{ST}^\text{Hudson}$.
...
% The Pool-seq corrected equations for $\text{\textit{F}}_\text{ST}$ available in the pdf document of PoPoolation's code repository are not implemented, to the best of our knowledge, in \toolname{PoPoolation} nor in \toolname{PoPoolation2} software.
